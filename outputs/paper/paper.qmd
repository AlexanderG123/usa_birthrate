...
Analysis of data “The Puzzle of Falling US Birth Rates since the Great Recession” [@citepaper], a paper from the Journal of Economic Perspectives [@citewebsite], was done using the R programming language [@citeR] in conjunction with the following packages: tidyverse [@tidyverse] and Knitr [@knitr]. The figures were created using ggplot2 [@citeggplot]. Data points on birth rates per 1000 women was used to observe overarching birth patterns in the US from women ages 15-44 from 1980-2020. Demographic variables were also obtained from the data providing functionality for further analysis. Birth rates by age group, marital status and education level were available and selected as independant variables from the data. This breakdown facilitated a more nuanced understanding of how birth rates varied within these demographic cohorts for the specified period.

The paper is organized with a data, results and discussion section. In the data @sec-data, we describe the methodological approach including data sourcing and anylitical tools. In the results @sec-results we show our analysis with descriptive figures, and finally, in the discussion @sec-discussion we discuss the significance of the results within the context of the previous research, and areas of future study.
...
Other possibly influences on fertility decisions should also be examined at a more social level, not just economic. Changing policy and attitudes will constanly be a force in the number of children being born. Looking into how shock changes in societies structure would be a interesting topic of analysis. An example of a topic of research would be the impact of the Roe v. Wade political changes across statees in the US.
Medical advancements are another changing field, examining the implications of an aging population and the social and economic costs of altering fertility patterns could offer insightful information to policymakers and society at large.
